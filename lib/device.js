...
// [TODO] implementation of maintain
Device.prototype.maintain = function (callback) {   // callback is optional
    var self = this,
        devAttrs = _.keys(this._attrs) || [],
        nc = this.get('netcore'),
        freebird = nc ? nc._freebird : undefined;

    if (!nc || !freebird)
        return setImmediate(callback, new Error('Cannot maintain. Device is not registered properly'));

    if (!this.isEnabled() || this.get('status') === 'offline')
        return setImmediate(callback, null, false); // offline or disabled, not maintained
