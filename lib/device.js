...
Device.prototype.get = function (propName, arg) {
    var result;
    utils.validateAttrName(propName, 'propName should be a string.');
...
Device.prototype.set = function (propName, arg) {
    var delta;
    utils.validateAttrName(propName, 'propName should be a string.');

    switch (propName) {
        case '_id':     // _setId, protected
            utils.validateId(arg);
            this._id = arg;     // arg = id
            break;
        case '_raw':    // _setRawDev
            this._raw = arg;    // arg = raw
            break;
        case 'net':     // setNetInfo
            utils.validateInfo(arg, 'info should be an object');
            delta = utils.getObjectDiff(arg, this.get('net'));
            ...
            break;
        case 'props':   // setProps
            utils.validateInfo(arg, 'props should be an object');
            delta = utils.getObjectDiff(arg, this.get('props'));
            if (!_.isEmpty(delta)) {
            ...
            break;
        case 'attrs':   // setAttrs
            utils.validateInfo(arg, 'attrs should be an object');
            delta = utils.getObjectDiff(arg, this.get('attrs'));
            if (!_.isEmpty(delta)) {
...
Device.prototype.resetTraffic = function (dir) {
    ...
    if (!_.isUndefined(dir))
        utils.validateAttrName(dir, "dir should be a string 'in' or 'out' if given");
    ...
    this._fire(DEV_EVT.netChanged, {   // only report@reset
        data: { traffic: traffic }
    });
    return this;
};

// [TODO] no trim, but need tackle at freebird
Device.prototype.dump = function (trim) {
    ...
    if (trim) {
        delete data.net.maySleep;
        delete data.net.sleepPeriod;
        data.gads = _.map(data.gads, function (rec) {
            return rec.gadId;
        });
    }
...
Device.prototype.recoverFromRecord = function (rec) {
    utils.validateInfo(rec, 'rec should be an object.');
    var self = this,
...
Device.prototype._accumulateBytes = function (dir, num) {
    utils.validateAttrName(dir, 'dir should be a string.');
    utils.validateId(num, 'gadId should be a number or a string.');
...
Device.prototype._fire = function (evt, data) {
    utils.validateAttrName(evt, 'evt should be a string.');
    var nc = this.get('netcore'),
        emitted = false,
        emitData,
...
Device.prototype._callDriver = function (drvName, args) {
    utils.validateAttrName(drvName, 'drvName should be a string.');
    var nc = this.get('netcore'),
        driver = nc ? nc._findDriver('dev', drvName) : undefined,
...
Device.prototype.read = function (attrName, callback) {
    ...
    utils.validateAttrName(attrName);
    utils.validateCallback(callback);
