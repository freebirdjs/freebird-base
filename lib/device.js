...
function Device(netcore, rawDev) {
    ...
    Object.defineProperty(this, '_recovered', _.assign({ value: false }, baseProp));    // helper, tag if it is reload from database
    ...
    switch (name) {
        ...
        case 'props':   // setProps
            ...
            if (!_.isEmpty(delta)) {
                ...
                this._fire(EVTS.DevPropsChanged, { data: delta, _data: oldData });
            }
            break;
        case 'attrs':   // setAttrs
            ...
            if (!_.isEmpty(delta)) {
                ...
                this._fire(EVTS.DevAttrsChanged, { data: delta, _data: oldData });
            }
            break;
        default:
            break;
...
Device.prototype._recoverFromRecord = function (rec) {
    ...
    // recoverying, but no raw, no extra
    ...
    if (wasEnabled)         // if it was enabled before, enable it after recovered
        this.enable();
...
Device.prototype._fire = function (evt, data) {
    var nc = this.get('netcore'),
        ...
        emittable = isErrEvt || isRegEn;

    if (!emittable)
        return false;

    data = data || {};
    _.assign(emitData, data, { id: this.get('id') });
    
    return nc._fire(evt, emitData);
...
Device.prototype.read = function (attrName, callback) {
    ...
    if (err)
        ...
        return setImmediate(callback, 'Device driver read is not implemented');
    ...
    read(permAddr, attrName, function (err, data) {
        ...
        if (!err) {
            ...
            self._fire(EVTS.DevRead, { permAddr: permAddr, data: result });
        }
...
Device.prototype.write = function (attrName, val, callback) {
    ...
    if (err)
        ...
        return setImmediate(callback, 'Device driver write is not implemented');
    ...
    write(permAddr, attrName, val, function (err, data) {
        ...
        if (!err) {
            ...
            self.set('attrs', result);
            self._fire(EVTS.DevWrite, { permAddr: permAddr, data: result });
        }
...
Device.prototype.identify = function (callback) {
    ...
    if (err)
        ...
        return setImmediate(callback, 'Device driver identify is not implemented');

    identify(permAddr, function (err, data) {
        if (!err) 
            self._fire(EVTS.DevIdentify, { permAddr: permAddr });
...
Device.prototype.ping = function (callback) {
    ...
    if (!_.isFunction(ping))
        err = err || new Error('Net driver ping not found');
...
Device.prototype.maintain = function (callback) {
    ...
    if (_.isUndefined(callback)) {
        callback = function (err, maintained) {
            if (err)
