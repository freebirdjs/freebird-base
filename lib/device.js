...
function Device(netcore, raw) {
    ...
    this._lastTime = null;  // POSIX Time, seconds, nc should call dev._active() to update it
...
Device.prototype._isWorking = function () {
    return (this._netcore && !_.isNil(_id) && this._enabled);
};

Device.prototype._active = function () {
    this._lastTime = Math.floor(Date.now()/1000);
};
