...
function Device(netcore, rawDev) {
    ...
    this._removing = false;     // devLeaving helper, is it been removed by remove()?
...
Device.prototype.enable = function () {
    ...
    // should enable, or _fbEmit() will be blocked
...
Device.prototype.disable = function () {
    ...
    // should emit first then change enabled to false, or _fbEmit() will be blocked
    this._fbEmit('_dev:netChanged', {
        data: { enabled: false }
    });
...
Device.prototype.recoverFromRecord = function (rec) {
    ...
    // dev status should be ensured with 'unknown'
    rec.net.status = 'unknown';
    ...
    _.forEach(rec.gads, function (gRec) {
