...
/*************************************************************************************************/
Gadget.prototype.get = function (propName, arg) {
    var result;
...
Gadget.prototype.set = function (propName, arg) {
    var delta;
    utils.validateAttrName(propName, 'propName should be a string.');
    ...
    switch (propName) {
        case '_id':     // _setId, protected
            ...
            this._id = arg;     // arg = id
            this.get('dev')._setGadIdToAuxId(arg, this.get('auxId'));
            break;
        case '_dev':
            utils.validateDevice(arg);
            this._dev = arg;    // arg = dev
            break;
        case '_raw':
            this._raw = arg;    // arg = raw
            break;
        case 'panel':           // setPanelInfo
            utils.validateInfo(arg, 'Input info should be an object');
            delta = utils.getObjectDiff(arg, this.get('panel'));    // arg = info
            ...
            break;
        case 'props':           // setProps
            utils.validateInfo(arg, 'Input props should be an object');
            delta = utils.getObjectDiff(props, this.get('props'));  // arg = props
            ...
            break;
        case 'attrs':           // setAttrs
            utils.validateInfo(arg, 'Input attrs should be an object');
            delta = utils.getObjectDiff(attrs, this.get('attrs'));  // arg = attrs
...
Gadget.prototype.recoverFromRecord = function (rec) {
    utils.validateInfo(rec, 'Input rec should be an object');
...
Gadget.prototype.setReportCfg = function (attrName, cfg, callback) {
    ...
    utils.validateAttrName(attrName);
    utils.validateInfo(cfg, 'cfg should be a plain object.');
    utils.validateCallback (callback);
...
Gadget.prototype._dangerouslyAppendAttrs = function (attrs) {
    utils.validateInfo(attrs, 'Input attrs should be an object');
