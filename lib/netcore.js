...
function Netcore(name, controller, protocol, opt) {
    ...
    Object.defineProperty(this, '_state', _.assign({ value: NC_UNKNOW_STATE }, baseProp));
        ...
        value: function () {
            ...
            if (this._resetting) {
                ...
                this.start(function (err) {
                    ...
                    else if (_.isFunction(self.onReady))
                        self.onReady();
                });
...
Netcore.prototype.stop = function (callback) {
    ...
    cb = function (err) {
        ...
        if (!err) {
            ...
            self._state = NC_UNKNOW_STATE;
            self.disable(); // Must diable afrer NcStopped emitted, or event will be blocked
        } else {
            if (err.message === 'Netcore not enabled')
                force = true;
            self._fireup(BTM_EVTS.NcError, { error: err }, force);
        }
...
Netcore.prototype.reset = function (mode, callback) {
    ...
    cb = function (err) {
        if (!err) {
            ...
            self._resetting = false;
            self.enable();
            self._fireup(BTM_EVTS.NcError, { error: err }, true);
        }
...
/*************************************************************************************************/
Netcore.prototype._fireup = function (evt, data, force) {
    var emitData = {};
