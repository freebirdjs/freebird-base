...
Netcore.prototype._startJoinTimer = function (duration) {
    ...
    if (this._joinTicks) {
        this._joinTimer = setInterval(function () {
            ...
            self._fbEmit(NC_EVT.permitJoin, { timeLeft: self._joinTicks });
            
            if (self._joinTicks === 0) 
                self._clearJoinTimer();
        }, 1000);
    } else {
        this._fbEmit(NC_EVT.permitJoin, { timeLeft: 0 });
    }
...
Netcore.prototype.disable = function () {
    ...
    this.permitJoin(0, function (err, result) {
        if (err) {
            self._fbEmit(NC_EVT.error, { error: err });
        } else if (self.isEnabled()) {
            self._net.enabled = false;
            self._fbEmit(NC_EVT.disabled);
        }
    });
...
Netcore.prototype.commitDevNetChanging = function (permAddr, changes) {
    ...
    if (!this.isEnabled() || this.isBlacklisted(permAddr)) { // nc disabled or dev is blocked, ignore all messages
        committed = false;
        return committed;
    } else {
        ...
        this._fbEmit(NC_EVT.devNetChanging, { permAddr: permAddr, data: changes });
    }
...
Netcore.prototype.commitDevIncoming = function (permAddr, rawDev) {
    ...
    if (!this.isEnabled()) { // nc disabled, ignore all messages
        ...
        this._fbEmit(NC_EVT.bannedDevIncoming, { permAddr: permAddr, raw: rawDev });
    } else {
        ...
        this._fbEmit(NC_EVT.devIncoming, { permAddr: permAddr, raw: rawDev });
    }
...
Netcore.prototype.commitDevLeaving = function (permAddr) {
    ...
    if (!this.isEnabled()) {
        ...
        this._fbEmit(NC_EVT.devLeaving, { permAddr: permAddr }); // unregister@fb, kill instance
        committed = true;
    }
...
Netcore.prototype.commitGadIncoming = function (permAddr, auxId, rawGad) {
    ...
    if (!this.isEnabled())  {
        ...
        this._fbEmit(NC_EVT.bannedGadIncoming, { permAddr: permAddr, auxId: auxId, raw: rawGad });
    } else {
        // tell fb here comes a new gadget, register@fb
        this._fbEmit(NC_EVT.gadIncoming, { permAddr: permAddr, auxId: auxId, raw: rawGad });
    }
...
Netcore.prototype.commitDevReporting = function (permAddr, devAttrs) {
    ...
    if (!this.isEnabled()) {
        ...
        this._fbEmit(NC_EVT.bannedDevReporting, { permAddr: permAddr, data: devAttrs });
    } else {
        // user should commit devAttrs with correct format
        this._fbEmit(NC_EVT.devReporting, { permAddr: permAddr, data: devAttrs });
    }
...
Netcore.prototype.commitGadReporting = function (permAddr, auxId, gadAttrs) {
    ...
    if (!this.isEnabled()) {
        ...
        this._fbEmit(NC_EVT.bannedGadReporting, { permAddr: permAddr, auxId: auxId, data: gadAttrs, appendFlag: false });
    } else {
        // user should commit devAttrs with correct format
        this._fbEmit(NC_EVT.gadReporting, { permAddr: permAddr, auxId: auxId, data: gadAttrs });
    }
...
Netcore.prototype.dangerouslyCommitGadReporting = function (permAddr, auxId, gadAttrs) {
    ...
    if (!this.isEnabled()) {
        ...
        this._fbEmit(NC_EVT.bannedGadReporting, { permAddr: permAddr, auxId: auxId, data: gadAttrs });
    } else {
        // user should commit devAttrs with correct format
        this._fbEmit(NC_EVT.gadReporting, { permAddr: permAddr, auxId: auxId, data: gadAttrs, appendFlag: true });
    }
...
Netcore.prototype.start = function (callback) {
    ...
    if (!areDrvsReady) {
        ...
        this._fbEmit(NC_EVT.error, { error: err });
        return callback(err);
        ...
        this._callDriver('net', 'start', args, callback, function (err, result) {
            if (!err) {
                ...
                self.enable();
                self._fbEmit(NC_EVT.ready);
                self._fbEmit(NC_EVT.started);
            }
        });
    }
};
...
Netcore.prototype.reset = function (mode, callback) {
    ...
    args.push(mode ? 1 : 0);
    this.stop();                // this will stop and disable netcore
...
Netcore.prototype.remove = function (permAddr, callback) {
    ...
    this._callDriver('net', 'remove', args, callback, function (err, result) {
        if (!err)
            self._fbEmit(NC_EVT.devLeaving, { permAddr: permAddr });
    });
};
...
Netcore.prototype.ban = function (permAddr, callback) {
    ...
    if (_.isFunction(drvBan)) {
        drvBan(permAddr, function (er, result) {
            if (er) {
                callback(er);
                self._fbEmit(NC_EVT.error, { error: er });
            } else {
                self._block(permAddr);
                callback(null, permAddr);
                self._fbEmit(NC_EVT.netBan, { permAddr: permAddr });
            }
        });
    } else {
        self._block(permAddr);
    ...
    if (_.isFunction(drvUnban)) {
        drvUnban(permAddr, function (er, result) {
            if (er) {
                ...
                callback(null, permAddr);
                self._fbEmit(NC_EVT.netUnban, { permAddr: permAddr });
            }
        });
    } else {
        self._unblock(permAddr);
...
Netcore.prototype.ping = function (permAddr, callback) {
    ...
    this._callDriver('net', 'ping', args, callback, function (err, result) {
        if (!err)
            self._fbEmit(NC_EVT.netPing, { permAddr: permAddr, data: result });
    });
};
...
Netcore.prototype.devRead = function (permAddr, attrName, callback) {
    ...
    this._callDriver('dev', 'read', args, callback, function (err, result) {
        ...
        if (!err) {
            ...
            self._fbEmit(NC_EVT.devRead, { permAddr: permAddr, data: data });
        }
    });
};
...
Netcore.prototype.devWrite = function (permAddr, attrName, val, callback) {
    ...
    this._callDriver('dev', 'write', args, callback, function (err, result) {
        ...
        if (!err) {
            ...
            self._fbEmit(NC_EVT.devWrite, { permAddr: permAddr, data: data });
        }
    });
};
...
Netcore.prototype.identify = function (permAddr, callback) {
    ...
    this._callDriver('dev', 'identify', args, callback, function (err, result) {
        if (!err)
            self._fbEmit(NC_EVT.devIdentify, { permAddr: permAddr });
    });
};
...
Netcore.prototype.gadRead = function (permAddr, auxId, attrName, callback) {
    ...
    this._callDriver('gad', 'read', args, callback, function (err, result) {
        ...
        if (!err) {
            data = _.set(data, attrName, result);
            self._fbEmit(NC_EVT.gadRead, { permAddr: permAddr, auxId: auxId, data: data });
        }
    });
};
...
Netcore.prototype.gadWrite = function (permAddr, auxId, attrName, val, callback) {
    ...
    this._callDriver('gad', 'write', args, callback, function (err, result) {
        ...
        if (!err) {
            ...
            self._fbEmit(NC_EVT.gadWrite, { permAddr: permAddr, auxId: auxId, data: data });
        }
    });
};
...
Netcore.prototype.gadExec = function (permAddr, auxId, attrName, args, callback) {
    ...
    this._callDriver('gad', 'exec', argms, callback, function (err, result) {
        ...
        if (!err) {
            data = _.set(data, attrName, result);
            self._fbEmit(NC_EVT.gadExec, { permAddr: permAddr, auxId: auxId, data: data });
        }
    });
};
...
Netcore.prototype.setReportCfg = function (permAddr, auxId, attrName, cfg, callback) {
    ...
    this._callDriver('gad', 'setReportCfg', args, callback, function (err, result) {
        ...
        if (!err) {
            data = _.set(data, attrName, result);
            self._fbEmit(NC_EVT.gadSetReportCfg, { permAddr: permAddr, auxId: auxId, data: data });
        }
    });
};
...
Netcore.prototype.getReportCfg = function (permAddr, auxId, attrName, callback) {
    ...
    this._callDriver('gad', 'getReportCfg', args, callback, function (err, result) {
        ...
        if (!err) {
            data = _.set(data, attrName, result);
            self._fbEmit(NC_EVT.gadGetReportCfg, { permAddr: permAddr, auxId: auxId, data: data });
        }
    });
};
