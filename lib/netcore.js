...
// ok
Netcore.prototype._findDevByAddr = function (permAddr, callback) {
    var finder = this.findDevByAddr;

    if (!_.isFunction(finder) && this.isRegistered())
        finder = this._fb.findDevByAddr;

    if (_.isFunction(finder))
        finder(permAddr, callback);
    else
        callback(new Error('No finder'), null);
};

// ok
Netcore.prototype._findGadByAddrAuxId = function (permAddr, auxId, callback) {
    ...
    if (_.isFunction(finder))
        finder(permAddr, auxId, callback);
    else
        callback(new Error('No finder'), null);
};
