...
Netcore.prototype.registerNetDrivers = function (drvs) {
    ...
    var self = this,
        mandatory = [ 'start', 'stop', 'reset', 'permitJoin', 'remove', 'ping' ],
        optional = [ 'ban', 'unban' ],
        lackof = [];
...
Netcore.prototype.registerGadDrivers = function (drvs) {
    ...
    var self = this,
        ...
        optional = [ 'exec', 'setReportCfg', 'getReportCfg', 'report' ],
        lackof = [];
    ...
    if (lackof.length) {
        ...
        throw new Error('Gadget driver(s) ' + lack + ' should be implemented.');
    }

    // attach drivers
    _.forEach(drvs, function (fn, key) {
        self._drivers.gad[key] = fn;
    });
