...
Netcore.prototype.remove = function (permAddr, callback) {
    ...
    cb = function (err, pAddr) {
        ...
        if (dev) {
            dev._removing = true;
            // self._fire(EVTS.NcDevLeaving, { permAddr: permAddr });   // [TODO] committed at low layer
        }
...
Netcore.prototype.commitReady = function () {
    this.enable();
    this._onReady();
    return this._fire(EVTS._NcReady, {});   // [TODO]
};
...
Netcore.prototype._fire = function (evt, data) {
    ...
    if (!this.isRegistered())   //  || !this.isEnabled() killed
        return false;   // not emitted

    // [TODO] if (evt === EVTS.NcDisabled)
