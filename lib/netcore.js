...
function Netcore(name, controller, protocol, opt) {
    ...
    /**************************************************************************************************/
    this.cookRawDev = null; // function(dev, rawDev, done) { done(err, dev); }
    this.cookRawGad = null; // function(gad, rawGad, done) { done(err, gad); }
...
Netcore.prototype.gadRead = function (permAddr, auxId, attrName, callback) {
    ...
    // [TODO] update attrs => should move to Gadget Class thus we can use dev.set()
...
Netcore.prototype.writeReportCfg = function (permAddr, auxId, attrName, cfg, callback) {
    var self = this,
    ...
    this._callDriver('gad', 'writeReportCfg', args, callback, function (err, result) {
        self._fire(EVTS.NcGadWriteReportCfg, { permAddr: permAddr, auxId: auxId, data: _.set({}, attrName, result) });
    });
};

Netcore.prototype.readReportCfg = function (permAddr, auxId, attrName, callback) {
    var self = this,
        args = [ permAddr, auxId, attrName ];

    validate.argTypes({ permAddr: permAddr, auxId: auxId, attrName: attrName, callback: callback });

    this._callDriver('gad', 'readReportCfg', args, callback, function (err, result) {
        if (!err)
            self._fire(EVTS.NcGadReadReportCfg, { permAddr: permAddr, auxId: auxId, data: _.set({}, attrName, result) });
    });
};
